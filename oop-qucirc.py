...
def make_KE_matx(dimensions):
  ...
  for dim in dimensions:
      dim["Δx"] = (dim["x_max"] - dim["x_min"] + 1)/dim["N"]
      dim["coeff"] = 1j*ħ/2/dim["m_eff"]/dim["Δx"]**2
  KE_matx_len = np.prod([dim["N"] for dim in dimensions])
  #let's make an array for each diag/set of 2 diags
  diag_list = []
  placement_list = []
  central = np.full(KE_matx_len, -2*sum(dim["coeff"] for dim in dimensions))
  ...
  for dim in dimensions:
      ...
      off_diag = np.full(KE_matx_len - dim_offset_factor, dim["coeff"])
      diag_list.append(off_diag)
      diag_list.append(off_diag)
      placement_list.append(dim_offset_factor)
      placement_list.append(-dim_offset_factor)
      dim_offset_factor *= dim["N"]
...
def wavefunction_class_test():
    wf = Wavefunction.init_gaussian((0, 1))
    plt.plot(np.real(wf.vectorize((-10,10,200))))
    plt.show()

    
def oop_ivp_evolve_time_dep_test1():
  """
  do a simple test of ivp_evolve.  static no pot'l.  The wavepacket slowly
  diffuses outwards, then wraps around at the periodic b.c., so you see a
  standing wave form in pdf.
  ...
  params["KE_matx"] = make_KE_matx(dimensions)
  r = oop_ivp_evolve_time_dep(params)
  ani = plot_time_dep_ψ(np.transpose(r.y), 
                        params, 
                        method="animate_2d",
                        num_pts = 100)
...
if __name__=='__main__':
    ...
    # test make_KE_matx
    dimensions = [{"x_max":1, "x_min":-1, "N": 3, "m_eff": 1}]
    correct_res = 1j * ħ/2 * np.array([[-2,1,0],
                                       [1,-2,1],
                                       [0,1,-2]])
    ...
    dimensions = [{"x_max":1, "x_min":-1, "N": 3, "m_eff": 1},
                  {"x_max":1, "x_min":-1, "N": 3, "m_eff": 1}]
    correct_res = 1j * ħ/2 * np.array([[-4,1,0,1,0,0,0,0,0],
                                       [1,-4,1,0,1,0,0,0,0],
                                       [0,1,-4,1,0,1,0,0,0],
                                       [1,0,1,-4,1,0,1,0,0],
                                       [0,1,0,1,-4,1,0,1,0],
